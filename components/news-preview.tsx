...
export function NewsPreview() {
  ...
  useEffect(() => { 
    fetch("/api/news")
      ...
      .then((news: ManagedNews[]) => setItems(news.slice(0, 3)))
      .catch(() => setItems([])); 
  }, []);
    ...
    <div className="mt-10 grid gap-5 md:grid-cols-3">
      {items.map((news) => (
        <Link 
          href={`/news#${news.slug}`} 
          key={news.id} 
          className="panel cut p-6 transition duration-300 hover:-translate-y-2 hover:border-[#84955a] hover:shadow-glow flex flex-col"
        >
          {news.image && (
            <img 
              src={news.image} 
              alt={news.title} 
              className="mb-4 h-36 w-full rounded object-cover border border-white/10" 
            />
          )}
          <p className="text-xs text-[#acbd75]">{news.date}</p>
          <h3 className="mt-2 text-lg font-bold">{news.title}</h3>
          <p className="mt-2 text-sm leading-relaxed text-stone-400 line-clamp-2">{news.text}</p>
        </Link>
      ))}
    </div>
  );
