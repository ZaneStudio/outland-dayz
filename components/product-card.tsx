...
export function ProductCard({ product }: { product: ExtendedProduct }) {
  ...
  const scale = product.imgScale ?? 1;
  const x = product.imgX ?? 0;
  const y = product.imgY ?? 0;
  ...
  return (
    <div className="group relative w-full flex flex-col items-center justify-between rounded-3xl bg-[#12160e] border border-white/10 pt-36 pb-8 px-6 min-h-[380px] transition-all duration-500 hover:-translate-y-2 hover:border-[#b6c980]/40 hover:shadow-2xl hover:shadow-[#b6c980]/10">
      ...
      {/* Картинка, що виходить за рамки з твоїм масштабом та повноцінною анімацією наведення */}
        ...
        {product.image ? (
          <div className="h-36 w-36 flex items-center justify-center">
            <img
              ...
              className="max-h-36 max-w-full object-contain drop-shadow-[0_20px_20px_rgba(0,0,0,0.8)] transition-all duration-500 group-hover:scale-[calc(var(--sc)*1.1)] group-hover:translate-x-[var(--tx)] group-hover:translate-y-[calc(var(--ty)-10px)] group-hover:rotate-[-3deg]"
            />
          </div>
        ) : (
