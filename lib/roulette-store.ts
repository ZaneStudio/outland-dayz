...
export async function createRoulettePrize(input: Pick<RoulettePrize, "label" | "icon" | "image">) { const items = await getRoulettePrizes(); const item = { id: randomUUID(), ...input }; items.push(item); await save(items); return item; }
export async function updateRoulettePrize(id: string, input: Partial<Pick<RoulettePrize, "label" | "icon" | "image">>) { const items = await getRoulettePrizes(); const index = items.findIndex(item => item.id === id); if (index < 0) return null; items[index] = { ...items[index], ...input }; await save(items); return items[index]; }
