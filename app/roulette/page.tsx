...
export default function RoulettePage() {
  ...
  useEffect(() => { if (!user) return; fetch("/api/roulette/status", { cache: "no-store" }).then(r => r.json()).then(data => setPlayMinutes(Number(data.minutes) || 0)).catch(() => setPlayMinutes(0)); }, [user]);
  const spin = () => {
    if (!unlocked || playing) return;
    const index = Math.floor(Math.random() * prizes.length);
    setPlaying(true); setResult(null); setTurn(value => value + 5 + index);
    window.setTimeout(() => { setResult(prizes[index].label); setHistory(items => [prizes[index].label, ...items].slice(0, 3)); setPlaying(false); }, 3900);
  };
  ...
  return <main className="roulette-page min-h-[calc(100vh-64px)] border-b border-white/10"><div className="shell py-12 sm:py-20"><div className="grid items-center gap-12 lg:grid-cols-[.9fr_1.1fr]"><section className="animate-enter"><p className="eyebrow">Нагорода за виживання</p><h1 className="heading mt-3 text-6xl sm:text-7xl">Рулетка</h1><h2 className="mt-5 text-xl font-bold text-[#e3ebd4]">Грай на сервері та отримуй нагороди!</h2><p className="mt-4 max-w-lg leading-relaxed text-stone-300">Проведи 5 годин на сервері, щоб отримати безкоштовний оберт рулетки з цінними призами.</p>
        {user === undefined || (user && playMinutes === null) ? <div className="panel mt-9 animate-pulse p-5 text-stone-400">Завантажуємо ігрову статистику...</div> : !user ? <div className="panel cut mt-9 p-6"><LockKeyhole className="text-[#c5d983]" /><p className="mt-4 font-bold">Увійдіть через Steam</p><p className="mt-2 text-sm text-stone-400">Щоб враховувати ваш час у грі та відкривати нагороди.</p><Link href="/login" className="btn mt-5"><Ticket size={16} /> Увійти через Steam</Link></div> : <><div className="panel cut mt-9 p-5"><p className="eyebrow">Ваш час на сервері</p><div className="mt-4 flex items-center gap-3"><Clock3 className="text-[#9acb4c]" size={27} /><p className="text-2xl font-bold">{formatTime(playMinutes ?? 0)} <span className="text-stone-500">/ 05:00:00</span></p></div><div className="mt-5 h-2 overflow-hidden rounded-full bg-black/40"><div className="roulette-progress h-full rounded-full" style={{ width: `${Math.min(100, (playMinutes ?? 0) / requiredMinutes * 100)}%` }} /></div></div><p className="mt-6 font-semibold text-[#b7d778]">{unlocked ? "Доступний 1 безкоштовний оберт!" : `Зіграйте ще ${formatTime(requiredMinutes - (playMinutes ?? 0))} для доступу.`}</p><button onClick={spin} disabled={!unlocked || playing} className="btn roulette-spin-button mt-4 min-w-64 disabled:cursor-not-allowed disabled:opacity-45">{playing ? "Рулетка обертається..." : <><Sparkles size={17} /> Оберти рулетку</>}</button>{result && <div className="roulette-result animate-enter mt-5"><Gift size={19} /><span>Ви виграли: <b>{result}</b></span></div>}</>}</section>
