...
export default function RoulettePage() {
  ...
  useEffect(() => { if (!user) return; fetch("/api/roulette/status", { cache: "no-store" }).then(r => r.json()).then(data => { setPlayMinutes(Number(data.minutes) || 0); setSpinsAvailable(Number(data.rouletteSpins) || 0); }).catch(() => setPlayMinutes(0)); }, [user]);
