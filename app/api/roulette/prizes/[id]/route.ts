...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) { if (!await currentAdmin()) return NextResponse.json({ error: "Недостатньо прав" }, { status: 403 }); const { id } = await params; const body = await request.json(); const item = await updateRoulettePrize(id, { label: body.label ? String(body.label) : undefined, icon: body.icon ? String(body.icon) : undefined }); return item ? NextResponse.json(item) : NextResponse.json({ error: "Не знайдено" }, { status: 404 }); }
